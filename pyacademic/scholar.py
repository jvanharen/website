...
class Author:
    ...
    def __extractScholar(self, author):
        ...
        for i in range(0, len(tmpPubliArray)):
            ...
            id = (pq(tmpPubliArray[i]).attr(
                "data-href").replace(regexTerm, ""))[110:]
            url = 'https://scholar.google.com' + \
                (pq(tmpPubliArray[i]).attr("data-href").replace(regexTerm, ""))
            ...
            citedPapers += count
            self.publication[id] = {
                "scholar_id": id, "scholar_name": name, "scholar_url": url, "scholar_citecount": str(count)}
        ...
        for i in range(0, len(tmp_total_cite)):
            if (i == 0):
                ...
                gIndex = (citedPapers, sqrtCitations)[
                    sqrtCitations <= citedPapers]
            if (i == 2):
